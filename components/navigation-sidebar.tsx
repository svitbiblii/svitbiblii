...
export default function NavigationSidebar() {
  ...
  const normalizedCurrentPath = normalizePath(pathname.replace(/^\/(uk|en)/, '').split('#')[0].split('?')[0]);
        ...
        <div className="px-6 pt-1 pb-8">
          ...
          {showContent ? (
            <div className="overflow-y-auto max-h-[calc(100vh-180px)]">
              <p className="space-y-2 p-2 text-gray-600 dark:text-white">{t('content-text')}</p>
            </div>
          ) : (
            <div className="overflow-y-auto max-h-[calc(100vh-180px)]">
              {historyItems.length > 0 ? (
                    ...
                    <li key={item.id} className="mb-2">
                      <Link
                        ...
                        className={`block py-1 px-3 rounded-lg hover:bg-primary-lite transition-colors duration-200 ${
                          isActive(item.link) ? 'text-primary bg-primary-lite font-semibold' : 'text-muted-foreground'
                        }`}
                      >
                ...
                <div>
                  <p className="space-y-2 p-2 text-gray-600 dark:text-white">{t('noHistory')}</p>
                </div>
              )}
              {historyItems.length > 0 && (
                <button
...
//     );
    ...
    mainSidebarContent = (
      <div className="px-6 pt-1 pb-8">
        ...
        <p className="space-y-2 p-2 text-gray-600 dark:text-white">{t("defaultSidebarContent")}</p>
      </div>
    );
  }
  ...
  return (
    <div className={`relative h-screen ${sidebarWidthClass} overflow-y-auto pb-12 transition-all duration-300 ease-in-out bg-card`}>
      <button
      ...
      {expanded && ( // Рендеримо вміст сайдбару лише якщо він розгорнутий
        <div className="pt-12"> {/* Додано відступ зверху, щоб уникнути перекриття кнопкою */}
          {mainSidebarContent}
        </div>
      )}
    </div>
