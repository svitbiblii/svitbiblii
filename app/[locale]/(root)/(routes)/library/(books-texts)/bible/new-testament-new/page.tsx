...
const EpubReader = () => {
	...
	return (
		<div className="h-min-full flex">
			...
			<div
				className={`hidden h-screen w-72 min-w-72 overflow-y-scroll bg-secondary pb-12 shadow-lg ${
				...
				<div>
					...
					<div className="bg-secondary px-6 pt-1 pb-8">
						<div className="py-2 flex justify-between font-medium">
							<button
								className={`w-1/2 ${showContent ? '' : 'border-b-2 border-blue-500 text-blue-500'}`}
								onClick={() => {
			...
			<div>
				...
				<div className="navigation-controls mt-4 flex justify-between">
					<button
						className="md:flex items-center h-9 rounded-md px-3 border-4"
					...
					<button
						...
						disabled={!book || currentChapterIndex === book.spine?.length - 1}
					>
						наступна
					</button>
				</div>
