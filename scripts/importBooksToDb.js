  ...
  try {
    ...
    for (const book of booksData) {
      ...
      if (!book.id || !book.title || !finalSlug) {
        console.warn(`Skipping book due to missing required fields: ${JSON.stringify(book)}`);
    ...
    if (error.code === 'P2002') { // Prisma Client error code for unique constraint violation
      console.error('Unique constraint violation. This often means a duplicate ID or slug was found.');
      console.error('Check your JSON data for duplicate IDs or slugs, or ensure the slug generation is robust.');
    }
  } finally {
